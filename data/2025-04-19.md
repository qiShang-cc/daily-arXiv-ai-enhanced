...
<details>
  <summary>Details</summary>
Motivation: 随着6G系统中MIMO天线数量急剧增加，CSI压缩对减少反馈开销至关重要。现有学习模型泛化能力有限，难以适应不同场景的差异特性。

Method: 提出AiANet，融合注意力机制并行提取通道和空间特征，并引入局部感知自注意力机制捕捉全局和局部空间模式。采用混合训练方案增强跨场景泛化能力。

Result: 在相同场景下，AiANet的NMSE性能比现有AE方法提升3.42 dB；混合训练后，跨场景泛化能力显著优于基准方法。

Conclusion: AiANet通过注意力机制和混合训练，有效提升了CSI压缩性能及跨场景适用性，为6G系统提供了一种高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于已有的平方索引系数（SICs）压缩方法，作者希望进一步推广到更一般的矩形索引系数（RICs），以提高对特定频率DFT系数的计算效率。

Method: 提出了一个算法，通过仅使用O(N)次复数加法且无需复数乘法，将N点输入信号压缩为C点信号，并在压缩后的信号上计算DFT等价于原信号的RICs。

Result: 算法在N为2的幂时可与FFT结合，将所有RICs的计算复杂度降至O(C log C)。

Conclusion: 该算法特别适用于谐波分析等需要特定频率的场景，通过调整信号参数可快速计算目标频率。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI在无线通信中的标准化推进，研究其在CSI预测中的应用具有重要实践意义。

Method: 通过全面调查AI驱动的CSI预测，包括模型管理的实际操作（如训练、监控和数据收集）。

Result: 突出了AI在CSI预测中的关键因素及其在未来无线通信系统中的潜力。

Conclusion: 本文旨在为这一新兴领域的后续研究提供参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有皮下脑电设备仅适用于癫痫监测，缺乏适用于BCI的功能，因此需要开发符合BCI需求的设备。

Method: 设计并验证了SAFE系统，具有六通道、1024 Hz采样率、蓝牙低功耗传输和小体积（12x12 mm）。

Result: 系统在动物模型中表现出低噪声记录能力，支持未来BCI应用。

Conclusion: SAFE系统为慢性居家BCI奠定了基础，有望帮助残障人士。
...
<details>
  <summary>Details</summary>
Motivation: 现有MPC角度估计方法因DSS角度采样间隔限制分辨率。

Method: 利用PADP中相邻角度功率差与MPC偏移角的映射关系，改进MPC角度分辨率。

Result: 数值模拟显示角度和振幅估计误差降低一个数量级，接近CRLB。

Conclusion: 实验验证了该方法在实际应用中的优异性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索超大规模MIMO系统在无线边缘推理中的潜力，提出一种无需数字处理的新型框架。

Method: 将MIMO信道系数作为隐藏层节点，接收器模拟组合器作为可训练输出层，类比极限学习机框架。

Result: 该方法在性能上与深度学习和传统ELM相当，但复杂度大幅降低，适用于超低功耗设备。

Conclusion: 超大规模MIMO系统可作为神经网络应用于无线通信，实现高效边缘推理，推动通信与计算的融合。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决传统ISAC系统在感知精度、延迟和实时决策方面的挑战，DISAC框架旨在满足6G严格要求，推动动态复杂环境中的创新。

Method: DISAC采用分布式架构，包括智能数据处理、分布式协调和可重构智能表面（RIS）等新兴技术，支持先进数据表示、无缝目标切换和异构设备集成。

Result: 在智能工厂和道路安全用例中，DISAC显著提升了精度、安全性和操作效率，优于传统ISAC系统。

Conclusion: DISAC是下一代无线网络的基石，通过解决关键挑战，为动态复杂环境中的创新奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 随着低空经济的发展，复杂的物理环境对通信可靠性提出了更高要求。信道知识图（CKM）是6G网络中环境感知通信的关键技术之一，但如何高效构建3D CKM以捕获空间信道特性是一个挑战。

Method: 作者分析了城市低空场景中基站的潜在部署类型，并研究了3D信道增益模型。提出了一种基于3D-CGAN的CGM推断方法，仅需基站坐标即可推断CGM，无需额外测量。

Result: 仿真结果表明，3D-CGAN推断的CGM优于基准方案，能准确反映3D环境中的无线电传播条件。

Conclusion: 该方法显著减少了在线开销，为低空航空器的安全运行提供了可靠通信支持，是6G网络环境感知通信的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 随着ELAA系统中天线数量的增加，信号传播趋向近场球面波传播，传统波束码本设计面临挑战。需要一种新方法提升波束聚焦的精度。

Method: 采用Transformer架构的两阶段波束训练算法，包括粗搜索阶段（大致定位设备）和细搜索阶段（精细调整波束），使用不同精度的码本。

Result: 实验显示，该方法在20 dB SNR下的波束选择准确率达97%，比基线方法在不同SNR下提升了10%至50%。

Conclusion: 所提出的两阶段算法显著提升了RIS辅助波束聚焦的精度，为ELAA系统中的IoT通信提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为光学无线通信系统提供一种实用的物理层认证解决方案，确保LiFi网络的安全性。

Method: 利用LED的非线性特性，在时域和频域中生成基于幅度的特征向量(OFP模型)，并通过IEEE 802.15.7协议实现设备指纹提取。

Result: 在39个商用LED的实验中，分类准确率达到90.36%，且符合标准要求。

Conclusion: OFP模型为可见光通信提供了一种有效的硬件级安全框架，性能优越且易于实现。
...
### [23] [Simultaneous Polysomnography and Cardiotocography Reveal Temporal Correlation Between Maternal Obstructive Sleep Apnea and Fetal Hypoxia](https://arxiv.org/abs/2504.13010)
*Jingyu Wang,Donglin Xie,Jingying Ma,Yunliang Sun,Linyan Zhang,Rui Bai,Zelin Tu,Liyue Xu,Jun Wei,Jingjing Yang,Yanan Liu,Huijie Yi,Bing Zhou,Long Zhao,Xueli Zhang,Mengling Feng,Xiaosong Dong,Guoli Liu,Fang Han,Shenda Hong*
...
<details>
  <summary>Details</summary>
Motivation: 研究母体缺氧对胎儿心率的直接影响，填补现有研究空白。

Method: 结合PSG和CTG数据，通过GLM分析母体缺氧事件与FHR变化的关联。

Result: 母体缺氧时长与FHR加速相关，缺氧持续时间和SpO2下降则与减速相关，FHR在缺氧后恢复基线。

Conclusion: 母体OSAS可能导致胎儿缺氧，强调了母婴交互的重要性，为干预提供依据。
...
<details>
  <summary>Details</summary>
Motivation: VLC由于链路阻塞问题导致连接中断，需要一种方法来缓解这些中断。光学可重构智能表面（ORISs）被认为是一种潜在解决方案。

Method: 研究多用户VLC系统，通过优化ORISs与LED和用户的关联配置，最小化中断概率并限制ORISs的使用数量。

Result: 数值结果表明，优化算法使用ORISs后，中断概率比未使用ORISs时减少了85%。

Conclusion: ORISs可以有效减少VLC系统中的中断概率，提升系统性能。
...
### [27] [Optimizing Utility-Scale Solar Siting for Local Economic Benefits and Regional Decarbonization](https://arxiv.org/abs/2504.12508)
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法在数据有限场景下性能不足，且缺乏同时保障数据隐私和容错的电池管理框架。

Method: 结合分散式群体学习框架和信誉权重模型合并机制，验证于66块商业锂镍钴铝氧化物电池数据。

Result: 在数据平衡、数量偏置、特征偏置和质量偏置四种场景下，群体学习均能提升估计精度，并与大数据集中学习效果相当。

Conclusion: 群体学习为数据有限的电池容量估计提供了一种隐私安全且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决移动机械臂在组装建筑块时的准确拾取和放置问题，确保结构稳定性。

Method: 使用眼内手视觉伺服控制器和控制屏障功能（CBFs）跟踪对象，结合眼到手设置确保精确放置，融入人机协作能力。

Result: 框架通过实验验证，能够在六自由度移动机械臂上可靠完成任务。

Conclusion: 提出的框架有效解决了建筑块组装中的拾取和放置问题，并具备鲁棒性和灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 传统电力规划未直接将本地经济影响纳入容量扩展决策，且现有经济评估常忽视农地转用的机会成本。

Method: 研究将本地经济指标内生于多目标优化框架，结合供应与边际收益曲线，分析太阳能选址对经济的影响。

Result: 经济规模大的县太阳能收益最高（如俄亥俄州达34,500美元/兆瓦），考虑农业机会成本后收益减少16%。优先投资高回报县可提升总收益11%，系统成本仅增0.5%。

Conclusion: 将经济因素纳入太阳能规划能更好协调脱碳目标与区域经济发展。
...
<details>
  <summary>Details</summary>
Motivation: 卡尔曼滤波器的性能依赖于准确的系统建模和噪声统计特性，但在实际应用中难以获得。深度学习的强大非线性建模能力为改进卡尔曼滤波器提供了新机会。

Method: 通过脉冲神经网络（SNN）直接近似最优增益矩阵，设计SNN与卡尔曼滤波器的集成策略，减轻复杂矩阵运算的计算负担。

Result: 与传统方法相比，平均误差降低了18%-65%，同时保持了状态估计的准确性和鲁棒性。

Conclusion: SNN优化卡尔曼滤波器的方法显著减少了噪声依赖，提高了计算效率和估计精度，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 探索在安全关键应用中，利用数据驱动的DNN替代传统物理模型，实现高效准确的状态估计。

Method: 使用LSTM节点训练DNN模型，并通过MHE结合简化动态模型，形成非线性优化问题（NLP）进行求解。

Result: 模拟实验表明，该方法在噪声或传感器故障下仍能准确估计温度，且具备嵌入式硬件的实时能力。

Conclusion: DNN-based MHE框架在实时安全关键应用中展现出潜力，融合了模型与数据驱动的优势。
...
<details>
  <summary>Details</summary>
Motivation: 可再生能源（RES）的间歇性和不确定性需要电力系统的灵活性，当前电价与碳排放强度不匹配可能导致用户在碳排放高峰期用电。

Method: 提出了基于激励性电价的三层优化模型，用户提交灵活性投标并获奖励，市场运营平台与终端用户动态互动。

Result: 仿真结果表明，该框架有效引导用户低碳用电行为。

Conclusion: 框架成功将用户用电行为与碳排放趋势对齐，提升了电力系统的灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习技术在复杂动态系统建模与控制中的应用增加，如何在安全关键领域（如自动驾驶、机器人和医疗决策）确保系统的安全性和稳定性成为关键问题。

Method: 论文探讨了嵌入安全约束的多种方法，包括结构性先验、Lyapunov函数、控制屏障函数、预测控制、投影技术和鲁棒优化，并结合不确定性量化与安全验证工具（如可达性分析和神经网络验证）。

Result: 通过实例展示了如何将数据驱动方法与物理原理结合，实现复杂动态系统的安全控制。

Conclusion: 论文为安全物理信息机器学习提供了全面的实现路径，强调了在复杂动态系统中确保安全性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决高自由度机械臂在复杂运动中遇到的抖动、未知干扰和旋转运动跟踪挑战，提高控制精度和鲁棒性。

Method: 采用自适应增益的非奇异终端超螺旋滑模控制（NT-STSM）方法，并通过仿真和硬件实验验证。

Result: 控制器在未知干扰下表现出更高的跟踪精度和鲁棒性，同时减少了控制抖动和不稳定性。

Conclusion: 提出的NT-STSM控制器适合用于高自由度机械臂的灵巧操作和工业应用。
...
<details>
  <summary>Details</summary>
Motivation: 近端梯度方法虽易于实现，但存在解偏差和收敛慢的问题，需通过超参数控制改进。

Method: 设计积分控制器调节超参数，保持计算复杂度不变，分析其在强凸问题中的收敛性。

Result: 理论分析和数值模拟验证了该方法在强凸和非强凸问题中的有效性。

Conclusion: 积分控制显著提升近端梯度方法的性能，实现无偏解且收敛更快。
...
### [51] [Learning Transferable Friction Models and LuGre Identification via Physics Informed Neural Networks](https://arxiv.org/abs/2504.12441)
*Asutay Ozmen,João P. Hespanha,Katie Byl*

Main category: cs.RO

TL;DR: 提出了一种结合物理模型与可学习组件的摩擦估计框架，仅需少量数据即可准确模拟动态摩擦特性，减少仿真与现实的差距，并具有跨系统泛化能力。


<details>
  <summary>Details</summary>
Motivation: 机器人模拟器中简化的摩擦模型可能导致仿真与实际性能的显著差异，需要一种既能保持物理一致性又能适应现实复杂性的方法。

Method: 开发了一种物理信息驱动的摩擦估计框架，结合已知摩擦模型与可学习组件，仅需少量通用数据即可训练。

Result: 在欠驱动和非线性系统上，通过学习到的摩擦模型准确模拟动态摩擦特性，显著减少仿真与现实的差距，并展现出跨系统泛化能力。

Conclusion: 该方法为机器人和控制领域提供了一种可扩展、可解释的摩擦建模路径，有效弥补仿真与现实的差距。
...
### [52] [Practical Insights on Grasp Strategies for Mobile Manipulation in the Wild](https://arxiv.org/abs/2504.12512)
*Isabella Huang,Richard Cheng,Sangwoon Kim,Dan Kruse,Carolyn Matl,Lukas Kaul,JC Hancock,Shanmuga Harikumar,Mark Tjersland,James Borders,Dan Helmick*

Main category: cs.RO

TL;DR: SHOPPER机器人平台被开发用于提升在非结构化环境中的抓取可靠性，通过在杂货店的实际部署验证其抓取策略的通用性。


<details>
  <summary>Details</summary>
Motivation: 解决现有移动操作机器人在非结构化环境中抓取不可靠的问题，推动其在真实世界的广泛应用。

Method: 设计通用抓取策略，并在具有多样物品和布局的实际杂货店环境中进行实地测试。

Result: 通过数百次抓取尝试的分析，揭示了关键失败模式，为机器人领域提供了实用见解。

Conclusion: 该研究不仅验证了通用抓取策略的可行性，还指出了未来研究的重点方向。
...
### [53] [Graph-based Path Planning with Dynamic Obstacle Avoidance for Autonomous Parking](https://arxiv.org/abs/2504.12616)
*Farhad Nawaz,Minjun Sung,Darshan Gadginmath,Jovin D'sa,Sangjae Bae,David Isele,Nadia Figueroa,Nikolai Matni,Faizan M. Tariq*

Main category: cs.RO

TL;DR: 论文提出了一种高效路径规划策略，通过改进的Hybrid A*算法结合动态障碍物预测，提升了停车场景中的安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 停车场景中复杂的静态和动态障碍物使得路径规划困难，需要兼顾安全性与计算效率。

Method: 提出了一种时间索引的Hybrid A*算法，显式结合动态障碍物预测，并集成到在线规划框架中，以自适应选择中间目标。

Result: 在多样化的停车场景（如垂直、斜向和平行停车）中验证了方法的有效性，仿真显示其相比现有方法显著提升了效率与安全性。

Conclusion: 该方法不仅适用于停车场景的路径规划，还能通过动态障碍物预测进一步提升安全性和效率。
...
### [54] [Biasing the Driving Style of an Artificial Race Driver for Online Time-Optimal Maneuver Planning](https://arxiv.org/abs/2504.12744)
*Sebastiano Taddei,Mattia Piccinini,Francesco Biral*

Main category: cs.RO

TL;DR: 本文提出了一种在线时间最优轨迹规划的新方法，通过非线性模型预测控制（MPC）框架结合时间最小化和出口速度最大化，使人工赛车驾驶（ARD）能够实时调整驾驶风格。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过优化轨迹规划，使人工赛车驾驶模拟更接近人类驾驶员的驾驶风格选择（如早或晚顶点转向）。

Method: 采用非线性MPC框架，提出新的终端成本公式，基于前一步规划的轨迹，实现从早顶点到晚顶点转向的实时调整。

Result: 模拟结果表明，该方法可实现接近离线最小圈时（MLT）的解，且优于在线最小时间MPC解。

Conclusion: 该方法不仅高效，还为理解人类驾驶员选择不同转向策略的原因提供了新视角。
...
### [55] [Imperative MPC: An End-to-End Self-Supervised Learning with Differentiable MPC for UAV Attitude Control](https://arxiv.org/abs/2504.13088)
*Haonan He,Yuheng Qiu,Junyi Geng*

Main category: cs.RO

TL;DR: 该论文提出了一种结合学习型惯性里程计（IO）和可微分模型预测控制（d-MPC）的自监督学习框架，用于无人机姿态控制，通过双层优化实现了端到端训练，有效提高了性能。


<details>
  <summary>Details</summary>
Motivation: 传统控制方法因保守假设和参数调优困难而表现不佳，纯数据驱动方法则效率低且依赖大数据。因此，结合学习和模型控制的混合方法成为有潜力的解决方案。

Method: 采用自监督学习框架，将IO模块用于去噪IMU数据并预测姿态，再通过d-MPC优化控制动作，通过双层优化实现端到端训练。

Result: 即使在强风环境下也表现优异，同时提升了MPC参数学习和IMU预测性能。

Conclusion: 该框架结合了学习型感知和模型控制的优势，为非线性动力系统控制提供了高效且可解释的解决方案。
...
### [56] [A New Semidefinite Relaxation for Linear and Piecewise-Affine Optimal Control with Time Scaling](https://arxiv.org/abs/2504.13170)
*Lujie Yang,Tobia Marcucci,Pablo A. Parrilo,Russ Tedrake*

Main category: cs.RO

TL;DR: 提出一种针对带时间缩放的线性系统最优控制的半定松弛方法，扩展至分段仿射系统，并通过单一半定规划求解。


<details>
  <summary>Details</summary>
Motivation: 线性系统的时间缩放最优控制问题本质非凸，传统方法效率低。

Method: 选择部分非线性项并变量替换，结合图论建模分段仿射系统。

Result: 实现了紧致的凸松弛，计算负担轻。

Conclusion: 方法高效且适用范围广，尤其适合分段仿射系统的最优控制问题。
...
### [57] [Optimizing Movable Antennas in Wideband Multi-User MIMO With Hardware Impairments](https://arxiv.org/abs/2504.12885)
...
### [58] [Degrees of Freedom of Holographic MIMO -- Fundamental Theory and Analytical Methods](https://arxiv.org/abs/2504.13031)
...
### [59] [Optimizing Movable Antennas in Wideband Multi-User MIMO With Hardware Impairments](https://arxiv.org/abs/2504.12885)
*Amna Irshad,Emil Björnson,Alva Kosasih,Vitaly Petrov*

Main category: cs.IT

TL;DR: 该研究探讨了在宽带MIMO系统中使用可移动天线的性能优势，提出了一种优化天线位置的方法，并分析了其在不同条件下的数据率提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多局限于窄带场景，而可移动天线在宽带MIMO系统中的潜在优势尚未充分量化。

Method: 提出了一种考虑硬件失真的上行宽带系统模型，并通过粒子群优化调整天线位置以最大化平均总速率。

Result: 可移动天线的数据率提升取决于硬件失真水平、多路径环境丰富度和子载波数量，性能优于固定均匀阵列。

Conclusion: 研究揭示了可移动天线在未来宽带系统中的适用场景，为其实际应用提供了重要参考。
...
### [60] [Degrees of Freedom of Holographic MIMO -- Fundamental Theory and Analytical Methods](https://arxiv.org/abs/2504.13031)
*Juan Carlos Ruiz-Sicilia,Marco Di Renzo,Placido Mursia,Vincenzo Sciancalepore,Merouane Debbah*

Main category: cs.IT

TL;DR: 本文比较了全息多输入多输出（Holographic MIMO）技术中常用的两种分析方法（cut-set积分和自伴随算子），并讨论了它们的优缺点。


<details>
  <summary>Details</summary>
Motivation: 未来的6G网络有望采用全息多输入多输出（Holographic MIMO）技术，通过增加自由度（DoF）显著提升空间复用增益。研究这些技术的理论基础至关重要。

Method: 比较了两种分析方法：cut-set积分和自伴随算子，并详细描述了它们的特点。

Result: 分析了两种方法的优势和局限性。

Conclusion: 本文为理解全息多输入多输出技术的理论基础提供了有价值的工具和方法。
...
### [61] [Benchmarking Audio Deepfake Detection Robustness in Real-world Communication Scenarios](https://arxiv.org/abs/2504.12423)
...
### [62] [Benchmarking Audio Deepfake Detection Robustness in Real-world Communication Scenarios](https://arxiv.org/abs/2504.12423)
*Haohan Shi,Xiyu Shi,Safak Dogan,Saif Alzubi,Tianjin Huang,Yunxiao Zhang*

Main category: eess.AS

TL;DR: 该论文提出了一种新基准测试ADD-C，用于评估音频深度伪造检测系统在实际通信场景中的鲁棒性，并通过数据增强策略显著提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 现有音频深度伪造检测系统在实际通信场景中因音频编解码压缩和信道传输效应导致性能下降，亟需一种方法来测试和改进系统鲁棒性。

Method: 开发ADD-C数据集以模拟不同通信条件，提出数据增强策略以提高系统鲁棒性。

Result: 实验表明基线模型在ADD-C上性能显著下降，提出的增强策略显著提升了系统性能。

Conclusion: ADD-C基准测试可为未来构建实用且鲁棒的音频深度伪造检测系统提供支持。
...
### [63] [TimeCapsule: Solving the Jigsaw Puzzle of Long-Term Time Series Forecasting with Compressed Predictive Representations](https://arxiv.org/abs/2504.12721)
...
### [64] [TimeCapsule: Solving the Jigsaw Puzzle of Long-Term Time Series Forecasting with Compressed Predictive Representations](https://arxiv.org/abs/2504.12721)
*Yihang Lu,Yangyang Xu,Qitao Qing,Xianwei Meng*

Main category: cs.LG

TL;DR: 论文提出了一种名为TimeCapsule的模型，通过高维信息压缩简化了长期时间序列预测中的复杂技术，实现了多尺度建模和冗余减少，并在实验中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前长期时间序列预测模型过于复杂且性能不佳，作者希望通过简化核心设计（如冗余减少和多尺度建模）提升深度学习效率。

Method: 提出TimeCapsule模型，将时间序列建模为3D张量，利用模式生成捕获多模式依赖并压缩维度，通过JEPA架构监控预测表示学习。

Result: 实验表明TimeCapsule在多个基准测试中达到最先进性能。

Conclusion: TimeCapsule通过高维信息压缩统一了复杂技术，简化了模型设计并提升了预测性能。
